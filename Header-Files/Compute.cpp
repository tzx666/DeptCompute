...
double DeptComputer::resblenewton(double x, double y, int n, double (*comp1)(double)) {
	double t = (y -x) / n;
	double res = 0;
	for (int i = 0; i < n; i++) {
		int t1 = i + 1;
		res+= newtoncont(x+t * (t1-1), x+t * t1, 4, comp1);
	}
	return res;
}
double DeptComputer::Gauss(double x, double y, double (*comp1)(double)) {
	double t = (y - x) / 2.0;
	double t1 = (y + x) / 2;
	double t2 = (y - x) / (2.0 * sqrt(3));
	return t*(comp1(t1-t2)+ comp1(t1 + t2));
}
double DeptComputer::Gauss1(double x, double y,double (*comp1)(double)) {
	double t = (y - x) / 2.0;
	double t1 = (y + x) / 2;
	double t2 = ((y - x) * sqrt(0.6)) /2.0;
	return t * ((5.0/9*comp1(t1 - t2)) + (8.0/9*comp1(t1))+(5.0 / 9 * comp1(t1 + t2)));
}